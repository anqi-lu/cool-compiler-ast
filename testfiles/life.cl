...
class Board inherits IO { 
 ...
 size_of_board(initial : Str) : Int {
   initial.length()
 };

 board_init(start : Str) : SELF_TYPE {
...
class CellularAutomaton inherits Board {
	...
	(let num : Int <- board_size in
	{
	 ...
	 while i < num loop
           {
	    outStr(population_map.substr(i,columns));
	    outStr("\n"); 
	    i <- i + columns;
	   }
	 pool;
...
(* A cell will live if 2 or 3 of it's neighbors are alive. It dies 
   otherwise. A cell is born if only 3 of it's neighbors are alive. *)
    ...
    cell_at_next_evolution(position : Int) : Str {
...
(* This is where the background pattern is detremined by the user. More 
 ...
 option(): Str {
 {
  (let num : Int in
   ...
   outStr("\t1: A cross\n"); 
   outStr("\t2: A slash from the upper left to lower right\n");
   outStr("\t3: A slash from the upper right to lower left\n"); 
   outStr("\t4: An X\n"); 
   outStr("\t5: A greater than sign \n"); 
   outStr("\t6: A less than sign\n"); 
   outStr("\t7: Two greater than signs\n"); 
   outStr("\t8: Two less than signs\n"); 
   outStr("\t9: A 'V'\n"); 
   outStr("\t10: An inverse 'V'\n"); 
   outStr("\t11: Numbers 9 and 10 combined\n"); 
   outStr("\t12: A full grid\n"); 
   outStr("\t13: A 'T'\n");
   outStr("\t14: A plus '+'\n");
   outStr("\t15: A 'W'\n");
   outStr("\t16: An 'M'\n");
 ...
 prompt() : Bool { 
 {
  (let ans : Str in
   {
   outStr("Would you like to continue with the next generation? \n");
   outStr("Please use lowercase y or n for your answer [y]: ");
   ans <- inStr();
   outStr("\n");
   if ans = "n" then 
	false
 ...
 prompt2() : Bool { 
  (let ans : Str in
   {
   outStr("\n\n");
   outStr("Would you like to choose a background pattern? \n");
   outStr("Please use lowercase y or n for your answer [n]: ");
   ans <- inStr();
   if ans = "y" then 
	true
   else
...
class Main inherits CellularAutomaton {
    ...
    main() : SELF_TYPE {
	 ...
	 (let continue : Bool  in
	  (let choice : Str  in
	   {
	   outStr("Welcome to the Game of Life.\n");
	   outStr("There are many initial states to choose from. \n");
	   while prompt2() loop
	    {
	     continue <- true;
	     choice <- option();
